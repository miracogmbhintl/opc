    ...
    <div style={{ padding: '0', paddingBottom: '140px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {urgentItems.map((item) => (
              <div
                ...
                onClick={() => {
                  if (item.type === 'job') {
                    window.location.href = `${baseUrl}/miraka-co-portal/project/${item.id}`;
                  }
                }}
                style={{
                  display: 'flex',
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {upcomingJobs.map((job) => (
              <div
                key={job.job_id}
                onClick={() => (window.location.href = `${baseUrl}/miraka-co-portal/project/${job.job_id}`)}
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'space-between',
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px' }}>
          <ActionLink href={`${baseUrl}/miraka-co-portal/create-client`} icon={<Users size={16} />} label="Kunde anlegen" dark />
          <ActionLink href={`${baseUrl}/miraka-co-portal/create-project`} icon={<Plus size={16} />} label="Einsatz planen" />
          <ActionLink href={`${baseUrl}/miraka-co-portal/files`} icon={<Upload size={16} />} label="Dateien" />
          <ActionLink href={`${baseUrl}/miraka-co-portal/tickets`} icon={<Ticket size={16} />} label="Anfrage / Schaden" />
          <ActionLink href={`${baseUrl}/miraka-co-portal/clients/invoice`} icon={<FileSpreadsheet size={16} />} label="Rechnung / Bericht" full />
        </div>
      </div>
