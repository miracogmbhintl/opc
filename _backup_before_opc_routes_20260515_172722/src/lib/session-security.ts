...
async function forceLogout(message?: string) {
  try {
    ...
    if (typeof window !== 'undefined') {
      ...
      window.history.replaceState(null, '', '/miraka-co-portal');
      
      // Redirect to login
      window.location.replace('/miraka-co-portal');
    }
  } catch (error) {
 ...
 */
export async function secureLogout(redirectTo: string = '/miraka-co-portal') {
