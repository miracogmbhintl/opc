...
export const GET: APIRoute = async ({ request, locals }) => {
  try {
    ...
    if (!apiKey) {
      console.error('[opc/google-place-details] Google Places API key is missing.');
      return jsonResponse(
        {
          ok: false,
    ...
    if (!googleResponse.ok) {
      console.error('[opc/google-place-details] Google request failed:', googleResponse.status);
      return jsonResponse(
        {
          ok: false,
          error: 'Adresse konnte nicht geladen werden.',
        },
    ...
    console.error('[opc/google-place-details] failed:', error);
    return jsonResponse(
      {
        ok: false,
        error: 'Adresse konnte nicht geladen werden.',
