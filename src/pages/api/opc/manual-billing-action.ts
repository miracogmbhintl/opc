...
async function assertOwner(admin: any, userId: string) {
  ...
  if (!allowed) {
    throw new Error('Nur Inhaber dürfen Rechnungsfreigaben bearbeiten.');
  }
}
...
export const POST: APIRoute = async ({ request, locals, cookies }) => {
  try {
    ...
    if (!token) {
      return jsonResponse({ ok: false, error: 'Nicht angemeldet.' }, 401);
    }
    ...
    if (userResponse.error || !userResponse.data.user) {
      return jsonResponse(
        { ok: false, error: 'Sitzung ist ungültig oder abgelaufen.' },
        401,
      );
    }
    ...
    if (action === 'approve_manual') {
      queueUpdate = {
        ...
        blocker_message:
          reason || 'Vom Inhaber zur manuellen Rechnungsstellung freigegeben.',
        error_message: null,
        updated_at: now,
      };
      ...
      queueUpdate = {
        ...
        blocker_message:
          reason || 'Rechnungsstellung wurde vom Inhaber zurückgestellt.',
        error_message: null,
        updated_at: now,
      };
      ...
      queueUpdate = {
        ...
        blocker_message:
          reason || 'Rechnung wurde manuell erstellt und versendet.',
        error_message: null,
        updated_at: now,
      };
    ...
    console.error('[opc/manual-billing-action] failed:', error);
