import {
  buildFirstReminderHtml as buildFirstReminderHtmlCore,
  buildInvoiceHtml as buildInvoiceHtmlCore,
  buildPaymentReminderHtml as buildPaymentReminderHtmlCore,
} from './opc-document-html-core';
import type { OPCReminderOverrides } from './opc-document-html-core';
import type { OPCInvoicePdfInput } from './opc-document-pdf';

export * from './opc-document-html-core';
...
function normalizeInvoiceInput(input: OPCInvoicePdfInput): OPCInvoicePdfInput {
  ...
  return html.replace(
    ...
    (fullMatch, variableName: string, json: string) => {
      try {
        ...
        data.qrBill = {
          ...qrBill,
          // Swiss QR structured debtor data must be complete. If it is not,
          // omit the entire debtor block instead of generating an invalid QR code.
          debtor: complete
            ? {
                ...debtor,
                name,
...
export function buildPaymentReminderHtml(
  ...
  return validateQrDebtorInHtml(
    buildPaymentReminderHtmlCore(normalizeInvoiceInput(input), overrides),
  );
}
...
export function buildFirstReminderHtml(
  ...
  return validateQrDebtorInHtml(
    buildFirstReminderHtmlCore(normalizeInvoiceInput(input), overrides),
  );
}
