...
export default function OpcClientPortalAppV2({ section, itemId }: { section: PortalSection; itemId?: string }) {
  ...
  async function logout() { clearCachedOpcAuthProfile(); try { await supabase.auth.signOut({ scope: 'local' }); } finally { safeNavigate(buildUrl('/'), { replace: true }); } }
