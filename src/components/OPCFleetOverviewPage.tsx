...
import { CarFront, ClipboardList, Map as MapIcon, RefreshCw, Search, Wrench,
  ...
  return (
    <div style={{ ...opcCardStyle, padding: '18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap' }}>
      <div>
        ...
        <div style={{ marginTop: '5px', color: OPC_BRAND.muted, fontSize: '13px', fontWeight: 620 }}>{syncMessage || 'Noch keine Fahrzeuge verbunden. Fahrzeugdaten erscheinen hier nach der AutoAid-Anbindung.'}</div>
      </div>
...
export default function OPCFleetOverviewPage() {
  ...
  async function runSync() {
    /*
     * AutoAid is intentionally not connected yet.
     * Keep the fleet UI operational without treating
     * the missing external API as a system error.
     */
    setSyncMessage(
      'AutoAid API noch nicht verbunden. Die Live-Synchronisation wird aktiviert, sobald die Zugangsdaten verfügbar sind.'
    );
