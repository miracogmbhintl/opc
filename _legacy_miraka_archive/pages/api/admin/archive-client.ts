...
export const POST: APIRoute = async ({ request, locals }) => {
  console.log('🔵 [API] archive-client: Request received');
  ...
  try {
    ...
    if (!profile || profile.role !== 'owner') {
      ...
      return new Response(JSON.stringify({
        success: false,
        error: 'Access denied - Only owner can archive clients'
      }), {
        status: 403,
        headers: { 'Content-Type': 'application/json' }
      });
    ...
    console.log('📦 [API] Archiving client:', {
      id: clientId,
    ...
    console.log('✓ [API] Client archived successfully. Archive ID:', archiveResult);
