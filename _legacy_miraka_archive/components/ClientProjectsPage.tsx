...
const ClientProjectsPage: React.FC<ClientProjectsPageProps> = ({ baseUrl = '' }) => {
  ...
  const handleRequestProject = () => {
    window.location.href = `${baseUrl}/kunde/request-project`;
  };
...
const getStatusDisplay = (status?: string | null): 'Aktiv' | 'Pausiert' | 'Inaktiv' => {
  const normalized = String(status || '').toLowerCase();

  if (normalized === 'active' || normalized === 'in_progress') return 'Aktiv';
  if (normalized === 'paused' || normalized === 'on_hold') return 'Pausiert';
                      ...
                      <div className="client-header">
                        <div className="client-title">
                          ...
                          <div className={`status-badge ${statusClass}`}>
                            {statusDisplay === 'Aktiv' && <div className="status-icon status-active" />}
                            {statusDisplay === 'Pausiert' && (
                              <Pause className="status-icon status-paused" size={14} />
                            )}
                            {statusDisplay === 'Inaktiv' && <div className="status-icon status-inactive" />}
