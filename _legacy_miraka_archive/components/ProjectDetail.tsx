...
export default function ProjectDetail({ projectId }: ProjectDetailProps) {
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      ...
      window.location.href = `${baseUrl}/einsaetze`;
    } catch (err: any) {
  ...
  if (loading) {
    return <PortalSkeleton variant="detail" />;
  }
    ...
    <>
      ...
      {previewFile && (
        <FilePreviewModal
          ...
          allFiles={(currentFiles?.filter(f => f.type === 'file' && f.storage_path) || []) as any}
          currentIndex={currentFiles?.filter(f => f.type === 'file' && f.storage_path).findIndex(f => f.id === previewFile.id) || 0}
          isOpen={showPreviewModal}
          onClose={handleClosePreview}
          onNavigate={(index) => {
            const filesOnly = currentFiles?.filter(f => f.type === 'file' && f.storage_path) || [];
            const currentIndex = filesOnly.findIndex(f => f.id === previewFile.id);
            if (index > currentIndex) {
              handleNavigateFile('next');
            } else if (index < currentIndex) {
              handleNavigateFile('prev');
            }
          }}
        />
      )}
